...
def build_transformer_model(input_shape, head_size=256, num_heads=4, ff_dim=4, num_transformer_blocks=5, dropout=0.25):
    inputs = Input(shape=input_shape)
    x = inputs

    for _ in range(num_transformer_blocks):
        x_attention = MultiHeadAttention(key_dim=head_size, num_heads=num_heads, dropout=dropout)(x, x)
        x = tf.keras.layers.Add()([x, x_attention])
...
if __name__ == "__main__":
    ...
    print(f"                       Model: Transformer - {len(all_fold_results)} Folds Completed                      ")
    print("=" * 80)
